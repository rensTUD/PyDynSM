...
class RayleighLoveRod(StructuralElement):
    """Class for Rayleigh-Love rod element."""
    ...
    def LocalStiffness(self, omega):
        ...
        K_local = np.array([[1j*(-Ir*nu**2*omega**2*rho + A*E)*(alpha_1*np.exp(-1j*alpha_2*L) - alpha_2*np.exp(-1j*alpha_1*L)) / (np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L)),
                             1j*(-Ir*nu**2*omega**2*rho + A*E)*(alpha_1 - alpha_2) / (-np.exp(-1j*alpha_2*L) + np.exp(-1j*alpha_1*L))],
                            [1j*(-Ir*nu**2*omega**2*rho + A*E)*np.exp(-1j*L*(alpha_1 + alpha_2))*(alpha_1 - alpha_2) / (-np.exp(-1j*alpha_2*L) + np.exp(-1j*alpha_1*L)),
                             -1j*(-Ir*nu**2*omega**2*rho + A*E)*(alpha_2*np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L)*alpha_1) / (np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L))]])

        return K_local

    def ElementWaveNumbers(self, omega):
        """
        Determine the wavenumbers of RayleighLove rod.

        Input:
            omega: array. Range of frequencies of analysis.
        Output:
            alpha_1, alpha_2,: values. Wavenumbers (2 because 2nd-order EOM)
        """
        rho = self.rho
        A = self.A
        E = self.E
        ...
        alpha_1 = (1 / (-Ir * nu**2 * omega**2 * rho + A * E) * np.sqrt(
            (-Ir * nu**2 * omega**2 * rho + A * E) * rho * A) * omega)
        alpha_2 = (-1/(-Ir * nu**2 * omega**2 * rho + A * E) * np.sqrt(
            (-Ir * nu**2 * omega**2 * rho + A * E) * rho * A) * omega)
    ...
    def LocalElementDisplacements(self, u_nodes_global, omega, num_points):
        """
        Calcualte local displacements u(s).
    ...
    def displacement(self, x, omega, C=None, u_node_local=None):
        """
        Get the transverse displacments of the Rayleigh-Love rod.
