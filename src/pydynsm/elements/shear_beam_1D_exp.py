...
@ElementFactory.ElementType('Shear Beam')#, required_parameters = ['rho', 'A', 'G', 'L'])
class Shear1D(StructuralElement):
    ...
    def LocalStiffness(self, omega):
        ...
        K_local = np.array([[1j*A*G*(np.exp(-1j*alpha_2*L)*alpha_1 - np.exp(-1j*alpha_1*L)*alpha_2)/(np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L)),
                             1j*A*G*(alpha_1 - alpha_2)/(-np.exp(-1j*alpha_2*L) + np.exp(-1j*alpha_1*L))],
                            [1j*A*G*(alpha_1 - alpha_2)*np.exp(-1j*L*(alpha_1 + alpha_2))/(-np.exp(-1j*alpha_2*L) + np.exp(-1j*alpha_1*L)),
                             -1j*A*G*(np.exp(-1j*alpha_2*L)*alpha_2 - np.exp(-1j*alpha_1*L)*alpha_1)/(np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L))]])
    ...
    def ElementWaveNumbers(self, omega):
        ...
        alpha_1 = (np.sqrt(rho/G)*omega)
        alpha_2 = (-np.sqrt(rho/G)*omega)
    ...
    def LocalDistributedLoad(self, q, omega):
        ...
        # TODO - check
        el = np.array([-1j*G*(np.exp(-1j*alpha_2*L)*alpha_1 - np.exp(-1j*alpha_1*L)*alpha_2)/(np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L))*q_x/rho/omega**2 + -1j*G*(alpha_1 - alpha_2)/(-np.exp(-1j*alpha_2*L) + np.exp(-1j*alpha_1*L))*q_x/rho/omega**2,
                       -1j*G*(alpha_1 - alpha_2)*np.exp(-1j*L*(alpha_1 + alpha_2))/(-np.exp(-1j*alpha_2*L) + np.exp(-1j*alpha_1*L))*q_x/rho/omega**2 + 1j*G*(np.exp(-1j*alpha_2*L)*alpha_2 - np.exp(-1j*alpha_1*L)*alpha_1)/(np.exp(-1j*alpha_2*L) - np.exp(-1j*alpha_1*L))*q_x/rho/omega**2])
    ...
    def LocalElementDisplacements(self, u_nodes_local, omega, num_points):
        ...
        Output:
            u: array. Amplitude of displacement
        """
