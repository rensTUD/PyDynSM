...
node2 = s1.CreateNode(L,L,dof_config = [['x'],['z'],['phi_y']])
...
p_z = lambda omega: 1 if omega == omega_f else 0
...
elem1.SetSection('EulerBernoulli Beam', {'E': E, 'A':A, 'rho':rho, 'Ib':1*I, 'Wb': W})
...
q_b = lambda omega: 1e7 if omega == omega_f else 0
...
s1.PlotElementDisplacements(disp,scale=10)
s1.PlotMoments(force,scale = 1e-8)